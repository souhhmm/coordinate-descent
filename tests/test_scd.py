...
with open("tests/config/test_scd.toml", "rb") as f:
...
def test_convergence():
    ...
    optimizer = SteepestCoordinateDescent([x], lr=config["test_convergence"]["lr"])
...
def test_theoretical_bound():
    ...
    x = torch.zeros(d, requires_grad=True)
    optimizer = SteepestCoordinateDescent([x], lr=1 / L)
    f = QuadraticFunction(A, b)

    initial_loss = f.loss(x).item()
    initial_gap = initial_loss - f_opt
    ...
    for i in range(num_iters):
        ...
        current = loss.item() - f_opt
        theoretical = (1 - mu / (d * L)) ** i * initial_gap

        assert (
            current <= theoretical + tol
        ), f"Empirical gap {current} exceeded theoretical bound {theoretical} in iter {i}"
...
def test_outlier_values():
    ...
    optimizer = SteepestCoordinateDescent([x], lr=config["test_outlier_values"]["lr"])
...
def test_type_errors():
    ...
    with pytest.raises(TypeError):
        SteepestCoordinateDescent([123], lr=0.1)
...
def test_lr_zero():
    ...
    optimizer = SteepestCoordinateDescent([x], lr=0.0)
...
def test_compare_cifar10():
    ...
    optimizer_rcd = SteepestCoordinateDescent(model_rcd.parameters(), lr=lr)
